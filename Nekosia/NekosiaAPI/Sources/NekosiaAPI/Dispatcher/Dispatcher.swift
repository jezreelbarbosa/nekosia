...
public typealias DispatcherCompletion = (Result<(data: Data, response: HTTPURLResponse), APIError>) -> Void

public protocol Dispatching {
    @discardableResult
    func call(endpoint: Endpointing, completion: @escaping DispatcherCompletion) -> URLSessionDataTask?
}
...
public final class Dispatcher: Dispatching {
    ...
    @discardableResult
    public func call(endpoint: Endpointing, completion: @escaping DispatcherCompletion) -> URLSessionDataTask? {
    ...
    func makeURLRequest(endpoint: Endpointing) -> URLRequest? {
    ...
    func makeDataTask(request: URLRequest, completion: @escaping DispatcherCompletion) -> URLSessionDataTask {
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            self?.handle(data: data, response: response, error: error, completion: completion)
        }
    }

    func handle(data: Data?, response: URLResponse?, error: Error?, completion: @escaping DispatcherCompletion) {
        if let error = error {
